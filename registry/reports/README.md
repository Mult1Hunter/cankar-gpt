...
- **snapshot** (corpus-quality.md, near-duplicates.md, dlib-reconcile.md,
  ...
  style.md, bpb.md, passages.md): computed from gitignored data/ + checkpoints/
  or live dLib state by `cankar corpus stats` / `dedup` / `reconcile-dlib` /
  `merge` / `cankar tokenizer eval` / `chunk` / `stats` / `cankar evals
  holdout-freeze` / `style-train` / `bpb-freeze` / `cankar pairs segment`; CI
  cannot regenerate them - do NOT fold them into `report --all` or the drift
  gate goes red.
