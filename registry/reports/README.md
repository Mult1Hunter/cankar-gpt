...
- **snapshot** (corpus-quality.md, near-duplicates.md, dlib-reconcile.md,
  ...
  style.md, bpb.md): computed from gitignored data/ + checkpoints/ or live dLib
  state by `cankar corpus stats` / `dedup` / `reconcile-dlib` / `merge` /
  `cankar tokenizer eval` / `chunk` / `stats` / `cankar evals holdout-freeze` /
  `style-train` / `bpb-freeze`; CI cannot regenerate them - do NOT fold them into
